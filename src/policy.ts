...
export function loadPolicy(): HardStopPolicy {
  try {
    const raw = readFileSync(POLICY_PATH, "utf-8");
    return JSON.parse(raw) as HardStopPolicy;
  } catch {
...
export function logBlocked(
  ...
  try {
    if (!existsSync(COMPLIANCE_DB)) return;
    const db = new Database(COMPLIANCE_DB, { readonly: false });
    db
      .prepare(
        `INSERT INTO compliance_log (session_key, trigger_type, trigger_detail, decision, agent, violation)
       VALUES (?, ?, ?, 'blocked', ?, 1)`
      )
...
export function buildApproval(
  toolName: string,
  agent: string
): NonNullable<import("./types.js").BlockResult["requireApproval"]> {
  return {
    title: `HARD STOP: ${toolName} → ${AGENT_DISPLAY[agent] ?? agent}`,
    description:
      `This tool must be dispatched to ${AGENT_DISPLAY[agent] ?? agent}, not executed directly.\n\n` +
