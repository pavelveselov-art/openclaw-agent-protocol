...
function detectAgents(): DetectedAgent[] {
  try {
    ...
    const agents: DetectedAgent[] = (config.agents?.list ?? []).map(
      (a: { id: string; identity?: { name?: string }; workspace?: string }) => ({
        id: a.id,
...
// ── Infer default routing from agent IDs/names ─────────────────────────────────
...
function inferDefaultRouting(agents: DetectedAgent[]): RoutingRule[] {
...
async function confirmRouting(
  ...
  const confirmed: RoutingRule[] = [];

  for (const rule of rules) {
    const answer = await ask(
      `  ${rule.tool.padEnd(20)} → ${rule.target.padEnd(16)} (${rule.description})\n  [${rule.target}] > `
    );
...
function buildPolicyJson(rules: RoutingRule[]): object {
  ...
  return {
    ...
    mode: "enforce",
    description:
      "Hard Stop policy — routes tool calls to the correct specialist agent. " +
...
function installPlugin(): void {
  ...
  writeFileSync(
    join(EXTENSION_DIR, "openclaw.plugin.json"),
    JSON.stringify(
      {
...
function ensureComplianceDb(): void {
